...
TEST_CASE("ChaCha20-Poly1305 RFC 8439 test vector", "[crypto]") {
    ...
    SECTION("seal produces correct ciphertext and tag") {
        ...
        REQUIRE(ChaCha20Poly1305::seal(key.data(), nonce.data(), aad.data(), aad.size(), plaintext.data(),
                                       plaintext.size(), out.data(), out.size()));
    ...
    SECTION("open decrypts correctly") {
        ...
        REQUIRE(ChaCha20Poly1305::open(key.data(), nonce.data(), aad.data(), aad.size(), combined.data(),
                                       combined.size(), decrypted.data(), decrypted.size()));
    ...
    SECTION("open rejects tampered ciphertext") {
        ...
        std::vector<std::uint8_t> decrypted(plaintext.size());
        REQUIRE_FALSE(ChaCha20Poly1305::open(key.data(), nonce.data(), aad.data(), aad.size(), combined.data(),
                                             combined.size(), decrypted.data(), decrypted.size()));
    }
...
TEST_CASE("ChaCha20-Poly1305 empty plaintext", "[crypto]") {
    ...
    REQUIRE(
        ChaCha20Poly1305::open(key.data(), nonce.data(), nullptr, 0, out, sizeof(out), decrypted, sizeof(decrypted)));
}

TEST_CASE("ChaCha20-Poly1305 derive_keys", "[crypto]") {
    std::array<std::uint8_t, 32> master_key{};
    for (std::size_t i = 0; i < 32; ++i)
        master_key[i] = static_cast<std::uint8_t>(i);
