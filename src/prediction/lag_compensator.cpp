...
std::uint16_t LagCompensator::get_rewind_tick(std::uint16_t client_tick, float client_rtt_ms) const {
    if (tick_rate_ <= 0.0f)
        return client_tick;
...
bool LagCompensator::get_entity_state(std::uint16_t tick, NetworkId entity, std::vector<std::uint8_t>& out) const {
    if (!history_)
        return false;
...
bool LagCompensator::get_all_entities(std::uint16_t tick, std::vector<EntitySnapshot>& out) const {
    if (!history_)
        return false;
