...
struct LoopbackHub::Impl {
    ...
    static void deliver_to(LoopbackTransport* target, const Address& from, const uint8_t* data, std::size_t len,
...
LoopbackTransport::LoopbackTransport(std::shared_ptr<LoopbackHub> hub) : impl_(std::make_unique<Impl>()) {
    if (hub) {
...
bool LoopbackTransport::connect(const Address& address) {
    if (impl_->connected || impl_->bound)
        return false;
    Address client_addr;
...
bool LoopbackTransport::send(const uint8_t* data, std::size_t length, PacketReliability reliability) {
    ...
    if (impl_->bound) {
        impl_->hub->broadcast(impl_->local_addr, impl_->local_addr, data, length, reliability);
        return true;
    }
    if (impl_->connected) {
        impl_->hub->deliver(impl_->remote_addr, impl_->local_addr, data, length, reliability);
        return true;
    }
    return false;
}

bool LoopbackTransport::send_to(const Address& address, const uint8_t* data, std::size_t length,
                                PacketReliability reliability) {
    if (!impl_->bound)
        return false;
    if (impl_->should_drop())
...
void LoopbackTransport::poll() {
    ...
    for (auto it = impl_->delayed_outbound.begin(); it != impl_->delayed_outbound.end();) {
        if (it->delivery_time <= now) {
            if (it->broadcast) {
                impl_->hub->broadcast(it->sender, it->sender, it->data.data(), it->data.size(), it->reliability);
            } else {
                impl_->hub->deliver(it->recipient, it->sender, it->data.data(), it->data.size(), it->reliability);
            }
    ...
    if (!impl_->pending_recv.empty()) {
        impl_->ready_recv.insert(impl_->ready_recv.end(), std::make_move_iterator(impl_->pending_recv.begin()),
                                 std::make_move_iterator(impl_->pending_recv.end()));
        impl_->pending_recv.clear();
    }
}

bool LoopbackTransport::pop_receive(uint8_t* buffer, std::size_t max_length, std::size_t& out_length,
                                    Address& out_sender) {
    if (impl_->ready_recv.empty())
        return false;
...
void LoopbackTransport::enqueue_pending(const Address& sender, const uint8_t* data, std::size_t len,
                                        PacketReliability reliability) {
    Impl::Packet pkt;
    pkt.sender = sender;
